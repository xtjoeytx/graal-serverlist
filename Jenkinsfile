		...
		docker.withRegistry("https://index.docker.io/v1/", "dockergraal") {
			...
			if (RUN_TESTS) {
				stage("Run tests...") {
					customImage.inside("") {
						try{
							sh "cd /tmp/listserver/build && ctest -T test --no-compress-output --output-on-failure"
						} catch(err) {
						...
						sh "mkdir -p ./test && cp -fvr /tmp/listserver/build/Testing ./test/Testing"
						dir("./test") {
							archiveArtifacts (
							...
							withCredentials([string(credentialsId: 'PREAGONAL_GITHUB_TOKEN', variable: 'GITHUB_TOKEN')]) {
								dir("./dist") {
									if (!env.CHANGE_ID) { // Don't run on PR's
										...
										try {
											sh "cat ../RELEASE_DESCRIPTION.txt | github-release release --user xtjoeytx --repo graal-serverlist --tag ${release_type_tag} --name \"ListServer ${release_type_tag}\" ${pre_release} --description -"
										} catch(err) {

										}
										sh "github-release upload --user xtjoeytx --repo graal-serverlist --tag ${release_type_tag} --name \"${files}\" --file ${files} --replace"
									}
								}
							}
						}
